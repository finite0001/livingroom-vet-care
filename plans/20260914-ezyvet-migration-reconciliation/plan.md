...
Weight evidence: migration `20260916033310` and its strict API/panel expose created/linked native weight receipts and independently paginated source-change acknowledgments. Exact snapshot equality, approved source-head currentness, household drift and local-value matching remain separate facts. Legacy page observations never recorded a head version, so historical head fidelity stays explicitly unknown even after an A-to-B-to-A source change. The local112-migration stack passed601 unit tests,33 browser tests,52 weight SQL assertions and446 combined Auth/Storage/runtime checks. [Validation evidence](../../docs/evidence/canonical-migration-weight-evidence-20260915.json) records exact source hashes and limitations. No hosted deployment or new populated restore is claimed. This records the weight-only candidate before combined-stack validation.
Identity approval evidence candidate: migration `20260916010000` reads the exact owned contact/animal observation and its manifest-selected approved mapping. Legacy observations have no retained head version, so even equal snapshots report unknown observed-version fidelity. Approved-source currentness, local edits and household reassignment are distinct facts. SQL/real API/browser and112-migration populated restore checks pass locally; CI and rollout remain pending. This records the identity-only candidate before combined-stack validation. Global reconciliation, operational resolutions and frozen reports remain pending.

Combined canonical receipt integration: commit `82d4a54` preserves identity migration `20260916010000` and weight migration `20260916033310` together, with113 total migrations. Both panels and actual Auth/runtime checks survive the merge. Local lint/typecheck/build,604 units and37 migration browser tests pass. The frozen complete disposable sequence passes454 checks (metadata50, originals121, maximum137, migration146), including30 identity and52 weight pgTAP assertions. Cleanup and all recorded migration/source hashes were verified. [Combined evidence](../../docs/evidence/canonical-migration-receipts-combined-20260915.json). No combined populated restore or hosted rollout is claimed; earlier112-migration receipts remain separate evidence.
