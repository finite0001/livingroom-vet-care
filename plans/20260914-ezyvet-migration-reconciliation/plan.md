...
Status: phase1 recovery implemented locally; phase2 scan-evidence projection is in progress. Baseline `b585f47`; staging has99 canonical migrations and the local candidate has102. This plan addresses the orchestration/reconciliation requirement in [phase3](../20260913-ezyvet-clinical-import/phase-03-complete-migration.md). Individual import/review/release workflows are implemented; their success is not whole-migration acceptance.
...
2. **Reconciliation projection — in progress.** The local per-binding scan projection counts observations, distinct identities and distinct snapshots from each canonical ledger, and separates traversal, page limits, parent/household currentness and unavailable historical attempts. It explicitly leaves provider totals, clinical outcomes and complete-coverage acceptance unknown/unverified. Mapping-source currentness, clinical/review/capture reconciliation, append-only attempt events and paginated item drill-down remain pending. Add append-only nonclinical resolution records and frozen report requests.
...
Extend phase2 with durable attempt events and exact review/capture reconciliation, followed by item drill-down and frozen reports. The manifest candidate is local only; do not infer a hosted deployment from passing local SQL checks. Before reserving another version, inspect local files and hosted ledger again. Update exact migration-count/hash fixtures deliberately; never relax their canonical-stack checks to accept arbitrary versions. No scope backfill from unreviewed historical runs.
