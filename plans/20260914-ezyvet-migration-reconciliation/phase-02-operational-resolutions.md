...
Status: **planned**. Priority: phase 2, after canonical identity/weight receipt branches are reconciled. This document specifies work; no implementation, deployment or acceptance is claimed.

Parent requirements: [full implementation contract](implementation-contract.md), especially phases 2–3; [overall plan](plan.md); [ledger map](ledger-map.md). This increment completes the operational disposition workflow only. Global outcome accounting, frozen reports and supervised cutover remain required follow-up work, not optional scope reductions.

## Current evidence and design choice

The checked implementation has immutable manifests/scopes, replaceable append-only child bindings, durable child attempt events, bounded source observations and per-resource receipt adapters. No operational resolution or migration report table/API exists. Original scope exclusions are immutable manifest declarations. Existing clinical approvals, corrections and acknowledgments have separate authority and must remain unchanged.
