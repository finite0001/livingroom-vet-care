...
Status: implemented on `codex/reviewed-vaccinations`, based on merged PR110; all45 targeted browser cases and the74-migration upgrade/restore passed. This is the next clinical increment after [consult-scoped intake](phase-03a-vaccination-intake.md). Living Room Vet remains primary; source access is read-only.
...
4. Integrate one dirty-state callback into the existing shared patient-chart blocker. The DVM review entrypoint is the patient chart, while Tools remains ADMIN intake. Preserve frozen UUID through navigation, clear actor pointers on signout/account change, and ignore late responses after patient changes.
...
Implementation evidence: [reviewed outside-vaccination workflow](../../../docs/features/reviewed-outside-vaccinations.md). Clinical sample acceptance and Dr. Edler review remain pending.
