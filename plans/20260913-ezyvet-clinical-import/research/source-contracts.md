...
## Attachment contract refresh — September 13, 2026

The public [Fetch an Attachment operation](https://developers.ezyvet.com/#get-attachment) documents GET `/v1/attachment`, OAuth client credentials with `read-attachment`, and query filters `id`, `active`, `created_at`, `modified_at`, `record_type` and `record_id`. Its parent examples include Animal, Contact and Consult. The response wraps records in `items[].attachment` and includes `id`, `active`, `created_at`, `modified_at`, `file_id`, `file_download_url`, `record_type`, `record_id`, `mime_type`, `name`, `primary_image` and `notes`. Pagination metadata is illustrated, but the operation-specific parameter list does not specify a maximum page size. An application request cap must not be described as a provider guarantee.

The current page lists GET `/v1/file/{id}` in its File navigation, linked to [get-file](https://developers.ezyvet.com/#get-file), but the downloaded HTML has no corresponding operation section describing its authorization or binary response. This is an incomplete download contract, not evidence that downloads or read-only access are unavailable. The attachment response's URL alone does not settle permitted hosts, redirects, token handling, expiry or content limits. No authorized attachment sample was accessed during this refresh. Public HTML was read directly after the web reader rejected its size.

[Phase 3d](../phase-03d-attachment-intake.md) specifies implementable metadata intake, deliberately limited to approved Animal parents. Byte capture remains required later; an API original must not be represented with the existing `staff_reviewed_manual_export_v1` provenance.
