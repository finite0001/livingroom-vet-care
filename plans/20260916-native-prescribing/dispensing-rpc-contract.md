...
Status: **proposed design only**. No migration reserved and no dispensing implementation/clinical acceptance claimed. Implements the remaining fill-slot, partial dispensing, stock/billing and pickup portion of [implementation-contract.md](implementation-contract.md), using [dispensing-ledger-audit.md](dispensing-ledger-audit.md). Refill117 must be packaged first. The practice must review slot-remainder forfeiture and physical dispensing/pickup terminology before live commissioning.
...
All quantities in contexts/receipts canonical fixed3 decimals, including allocation quantities; target/request retains original exact input strings and array order. Context hash covers every listed field (including target), not observed_at. No live lookup when rendering historic artifact. `NativeDispenseArtifact` is the existing renderer's exact shape: `{id,authorization_id,authorization_hash,fill_index,quantity,unit,dispensed_at,recorded_by:{user_id,name},invoice_id,lots:[{id,number,expires_on,quantity}]}`. Frozen staff name is server-derived. Artifact hash is SHA256 canonical JSONB text; adapter verifies artifact matches immutable authorization/allocation/event links before printing. Existing schema1 signed artifact unchanged. Current status warnings remain outside historical label; pickup is a separate current/history annotation, not relabeled dispensing time.
...
4. Patient FOR SHARE via current alert review, then invoice FOR UPDATE. **Product/lot ordering is an unresolved implementation gate:** the initial audited treatment pattern is selected lot rows FOR UPDATE in UUID order, then product FOR SHARE, but that pattern is not yet accepted as deadlock-safe (see below). Household/version read must be consistent with patient's locked relationship; use compatible client SHARE lock before invoice only after auditing all household writers.
...
All preview/save paths must use one proven compatible order; preview creates no slot or financial effects. Before implementation, resolve the three-way queued-exclusive case: receive holds product SHARE while waiting for a lot; dispensing/treatment holds that lot and requests product SHARE behind a queued catalog product UPDATE; catalog update waits for the receiver's SHARE. Compatible SHARE lock modes alone do not prove progress. Existing treatment's lot→product path shares this concern. Investigate harmonizing product SHARE→sorted lot UPDATE across affected writers, preserving patient SHARE→invoice UPDATE before these locks. Any amendment to existing receive/treatment/dispense writers must be additive, jointly reviewed and tested; do not silently exclude the race or claim current ordering is safe. **No dispensing SQL implementation is accepted until the protocol and real observed three-way test are resolved.**
