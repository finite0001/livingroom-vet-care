...
function SignatureSection() {
  ...
  useEffect(() => {
    if (signature !== undefined && signature !== null) setValue(signature);
  }, [signature]);
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <Pen className="h-4 w-4 text-primary" /> Email signature
        </CardTitle>
      </CardHeader>
...
function ClinicSettings() {
  ...
  useEffect(() => {
    ...
    if (settings.retention_purge_days)
      setPurgeDays(settings.retention_purge_days);
  }, [settings]);
  ...
  return (
    <>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Archive inactive conversations, then purge old archived ones.
              </p>
            </div>
            <Switch
              checked={isOn("retention_enabled")}
              onCheckedChange={toggle("retention_enabled", "Data retention")}
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <Clock className="h-4 w-4 text-primary" /> Care reminders
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Review vaccine and lab due plans, message wording and delivery
            policies in Care reminders. Provider delivery requires separate
            commissioning.
          </p>
