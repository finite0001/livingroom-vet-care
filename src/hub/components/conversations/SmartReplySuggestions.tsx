...
export function SmartReplySuggestions({ conversationId, onSelect }: SmartReplySuggestionsProps) {
  ...
  // Drop empty/whitespace suggestions so we never render a zero-width pill or call
  // onSelect with a blank string (which would leave the parent draft stuck).
  const visible = suggestions.filter((s) => s.trim());
  return (
    <div className="flex items-center gap-1.5 px-3 py-1.5 border-t bg-muted/30 overflow-x-auto">
      ...
      {visible.map((s, i) => (<button key={i} className="shrink-0 rounded-full border bg-background px-2.5 py-1 text-xs hover:bg-accent transition-colors max-w-[200px] truncate" onClick={() => onSelect(s)} title={s}>{s}</button>))}
      {!loading && visible.length === 0 && <span className="text-xs text-muted-foreground">Click to generate suggestions</span>}
    </div>
  );
}
