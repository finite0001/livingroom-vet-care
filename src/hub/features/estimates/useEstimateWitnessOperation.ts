...
/** A null recovery never releases a possibly delayed witnessed decision mutation. */
export function useEstimateWitnessOperation<
  Operation extends IdentifiedOperation,
  Receipt,
>(options: Options<Operation, Receipt>) {
  const key = `lrv:estimate-witness:mutation:v1:${options.identity}`;
  function readSaved(): Operation | null {
    ...
    if (
      ...
      throw new Error(
        "The saved witnessed decision request could not be verified. Keep this browser session for recovery.",
      );
  ...
  const [initial] = useState(() => {
    try {
      ...
      return {
        ...
        blocked: true,
        error:
          "The saved witnessed decision request could not be verified. Keep this session and resolve its recovery record before starting another operation.",
      };
    }
  });
  ...
  const [notice, setNotice] = useState(
    initial.pending
      ? "An earlier witnessed decision request needs recovery. Its exact reviewed values remain saved."
      : "",
  );
  ...
  function finish(operation: Operation, receipt: Receipt | null) {
    ...
    setNotice(
      receipt
        ? "The exact witnessed decision operation is confirmed in history."
