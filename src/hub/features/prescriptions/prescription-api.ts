...
export function createPrescriptionApi(client: PrescriptionRpc, actor: string, patientId: string) {
  ...
  return {
    ...
    async readPrint(prior: PrescriptionAuthorization) {
      authorization(prior, patientId, prior.id);
      const result = await rpc("read_native_prescription_print_v4", { p_authorization_id: prior.id, p_dispense_id: null }) as NativePrescriptionPrintV4;
      renderNativePrescriptionV4(result); if (result.dispense !== null) throw new Error("Order-only print contains an unexpected dispense");
