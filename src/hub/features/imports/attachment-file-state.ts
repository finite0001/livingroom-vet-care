...
export function parseAttachmentFileRecovery(value: unknown, expected: AttachmentFileIntent) {
  ...
  if (request.source_context === null) {
    ...
    return { status: "abandoned" as const, requestHash: null, worker: null, captured: false, fileSize: null, cleanupIntentHash: null, original: null };
  }
  ...
  return { status: saved.status, requestHash, worker, captured: !!saved.capture, fileSize: saved.capture ? saved.intent!.file_size : null, cleanupIntentHash: saved.intent?.intent_hash ?? null, original: saved.capture ? { ...saved.intent!, captureHash: saved.capture.capture_hash } : null };
}
