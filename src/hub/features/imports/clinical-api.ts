...
export const resourceSchema = z.enum(["consult", "history"]);
export type ClinicalResource = z.infer<typeof resourceSchema>;
...
export type ClinicalMapping = z.infer<typeof mappingSchema>;
...
export type ClinicalRun = z.infer<typeof runSchema>;
const cursorSchema = z.object({ before_at: date, before_id: uuid });
export type ClinicalCursor = z.infer<typeof cursorSchema>;
...
export type ClinicalCandidate = z.infer<typeof candidateSchema>;
interface ClinicalDatabase {
  public: {
...
const client = supabase as unknown as SupabaseClient<ClinicalDatabase>;
async function rpc(name: string, args: Record<string, unknown>) {
  const { data, error } = await client.rpc(name, args);
  if (error)
    throw new Error(
      "Clinical import data unavailable. Recheck the original run before continuing.",
    );
  return data;
}
...
export async function recoverClinicalRun(
  id: string,
  actor: string,
  mapping: ClinicalMapping,
  resource: ClinicalResource,
) {
  const value = await rpc("recover_ezyvet_clinical_run", {
    p_id: id,
...
export async function listClinicalRuns(
  actor: string,
  mapping: ClinicalMapping,
  resource: ClinicalResource,
  cursor: ClinicalCursor | null,
...
export async function listClinicalCandidates(
  mapping: ClinicalMapping,
  resource: ClinicalResource,
  cursor: ClinicalCursor | null,
) {
...
export async function stageClinicalPage(
  id: string,
  mapping: ClinicalMapping,
  resource: ClinicalResource,
) {
