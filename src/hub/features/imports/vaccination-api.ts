...
export const resourceSchema = z.literal("vaccination");
export type VaccinationResource = z.infer<typeof resourceSchema>;
...
export type VaccinationMapping = z.infer<typeof mappingSchema>;
const runSchema = z.object({
  ...
  created_at: date,
  updated_at: date,
  scope: z.enum(["consult_scoped", "legacy_unscoped"]),
  ...
  consult_payload_hash: z
    .string()
    .regex(/^[a-f0-9]{64}$/)
    .nullable(),
  consult_observed_head_version: z.number().int().positive().nullable(),
  consult_external_id: z.string().nullable(),
});
...
const candidateSchema = z.object({
  ...
  consult_payload_hash: z.string().regex(/^[a-f0-9]{64}$/),
  consult_observed_head_version: z.number().int().positive(),
  consult_external_id: z.string(),
  consult_head_version: z.number().int().positive(),
  consult_current_snapshot_id: uuid,
  consult_is_current: z.boolean(),
...
async function rpc(name: string, args: Record<string, unknown>) {
  ...
  if (error)
    throw new Error(
      "Vaccination import data unavailable. Recheck the original run before continuing.",
    );
  return data;
}
...
export function contextOf(c: unknown): VaccinationContext {
  const v = contextSchema.parse(c);
  return {
    consult_snapshot_id: v.consult_snapshot_id!,
    consult_payload_hash: v.consult_payload_hash!,
    consult_observed_head_version: v.consult_observed_head_version!,
  };
}
  ...
  if (
    r.scope === "consult_scoped"
      ? r.animal_link_id !== mapping.link_id ||
        ...
        !r.consult_external_id
      : r.animal_link_id !== null ||
        r.pet_id !== null ||
        r.client_id !== null ||
        r.animal_external_id !== null ||
        r.consult_snapshot_id !== null ||
        r.consult_payload_hash !== null ||
        r.consult_observed_head_version !== null ||
        r.consult_external_id !== null
  )
    throw new Error(
...
export async function recoverVaccinationRun(
  id: string,
  actor: string,
  mapping: VaccinationMapping,
  ...
  const value = await rpc("recover_ezyvet_vaccination_run", {
    p_id: id,
    p_animal_link_id: mapping.link_id,
    p_consult_snapshot_id: context.consult_snapshot_id,
    p_consult_payload_hash: context.consult_payload_hash,
    p_consult_observed_head_version: context.consult_observed_head_version,
  });
  ...
  if (
    r.id !== id ||
    (r.scope === "consult_scoped" &&
      (r.consult_snapshot_id !== context.consult_snapshot_id ||
        r.consult_payload_hash !== context.consult_payload_hash ||
        r.consult_observed_head_version !==
          context.consult_observed_head_version))
  )
...
export async function listVaccinationRuns(
  actor: string,
  mapping: VaccinationMapping,
  resource: VaccinationResource,
  cursor: VaccinationCursor | null,
  ...
  const p = z
    ...
    .parse(
      await rpc("list_ezyvet_vaccination_runs", pageArgs(mapping, cursor)),
    );
...
export async function listVaccinationCandidates(
  mapping: VaccinationMapping,
  resource: VaccinationResource,
  cursor: VaccinationCursor | null,
) {
...
const errors: Record<string, string> = {
  SOURCE_CONSULT_STALE:
    "This consult observation has changed. Recover the original run, then refresh consult evidence before starting another scan.",
  IMPORT_DISABLED:
...
export async function stageVaccinationPage(
  id: string,
  mapping: VaccinationMapping,
  context: VaccinationContext,
) {
