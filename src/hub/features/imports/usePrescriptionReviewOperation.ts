...
import type { PrescriptionReviewIntent } from "./prescription-review-operation-state";
/** The owning component is keyed by actor and patient, preventing late replies crossing charts. */
export function usePrescriptionReviewOperation(actor: string, pet: string) {
  const key = prescriptionReviewKey(actor, pet);
  const [loaded] = useState(() => {
    try {
      return { intent: readPrescriptionReviewIntent(key), error: "" };
    } catch {
      return {
        intent: null,
        error:
  ...
  function retain(value: PrescriptionReviewIntent) {
    if (value.actor !== actor || value.pet !== pet)
      throw new Error("Review identity differs");
    savePrescriptionReviewIntent(value);
