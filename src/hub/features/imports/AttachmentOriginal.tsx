...
interface Props { intent: AttachmentFileIntent; disabled: boolean; onBusy: (busy: boolean) => void; onVerified?: (hash: string | null) => void; }
interface Original { url: string; filename: string; captureHash: string; }
export function AttachmentOriginal({ intent, disabled, onBusy, onVerified }: Props) {
  ...
  useEffect(() => {
    alive.current = true;
    const clear = () => { onVerified?.(null); generation.current++; if (url.current) URL.revokeObjectURL(url.current); url.current = null; if (alive.current) setOriginal(null); };
  ...
  }, [intent.actor, onBusy, onVerified]);
  async function inspect() {
    if (lock.current || disabled) return;
    lock.current = true; onVerified?.(null); setBusy(true); onBusy(true); setNotice(""); setOriginal(null);
  ...
  return <section aria-label="Captured original inspection" className="space-y-2 border-t pt-3">
    ...
    <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={busy || disabled} onClick={() => void inspect()}>{busy ? "Verifying private original…" : "Verify original for inspection"}</Button>
      {original && <a className={buttonVariants({ variant: "secondary" })} href={original.url} onClick={() => onVerified?.(original.captureHash)} download={original.filename} rel="noopener noreferrer">Download verified original</a>}
    </div>
  </section>;
}
