...
The current gap rehearsal also reproduces the six observed extra direct execution grants on its generated local source before capturing the initial inventory. Migration4600 must remove those extras while retaining the intended staff/consent access; the final88-migration canonical comparison verifies convergence. This fixture changes only local function ACLs and records that reproduction in the protected result. The separate `supabase/tests/explicit_rpc_grants_upgrade.py` runner also proves correction and rollback without changing bodies, owners or global default ACLs.
...
The current fixture also admits two API originals into the dedicated chart family, preserves an explicit replacement, two DVM acknowledgments, one withdrawal and one scoped abandoned action. Restore checks exact actor-owned action recovery, chart history and both approved originals while retaining a separate reserved original. No patient-document promotion or release inclusion occurs.

Final 51→88 chart-review restore evidence is [`attachment-original-review-restore-local-20260914.json`](../../docs/evidence/attachment-original-review-restore-local-20260914.json). The matching full and final targeted SQL/lock-race receipts are [`attachment-original-review-sql-contention-local-20260914.json`](../../docs/evidence/attachment-original-review-sql-contention-local-20260914.json).
