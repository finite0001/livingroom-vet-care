...
No Edge runtime, providers, SMTP delivery or scheduler Vault secrets are commissioned. Auth uses the local mail catcher; fixture users are confirmed via admin API without requesting email. The restored outbox is empty. In the current scheduler stack, `pg_cron` is present with the reviewed timetable, but the verifier requires missing `project_url` and `scheduler_worker_key` Vault values and proves a dispatch attempt returns `configuration_missing` before any worker call. These are no-sends-by-construction checks, not a claim of provider delivery acceptance.
