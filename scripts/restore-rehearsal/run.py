...
assert len(versions) == len(set(versions)) == 115, 'Review canonical restore migration inventory'
assert {'20260916010000','20260916033310','20260916043949','20260916055043'} <= set(versions), 'Canonical identity, weight, resolution and native prescription migrations required'
...
new_versions = [f'20260914{v:02d}0000' for v in range(1,24)] + ['20260916000000','20260916010000','20260916033310','20260916043949','20260916055043']
if args.rehearse_staging_baseline:
    assert len(migration_files) == 115
...
if args.rehearse_observed_hosted_gaps:
    ...
    assert len(migration_files)==115 and len(initial_files)==51
