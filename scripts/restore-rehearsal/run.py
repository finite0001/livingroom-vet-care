...
assert len(versions) == len(set(versions)) == 127, 'Review canonical restore migration inventory'
...
if args.rehearse_staging_baseline:
    assert len(migration_files) == 127
...
if args.rehearse_observed_hosted_gaps:
    ...
    assert len(migration_files)==127 and len(initial_files)==51
