...
assert len(versions) == len(set(versions)) == 125, 'Review canonical restore migration inventory'
...
if args.rehearse_staging_baseline:
    assert len(migration_files) == 125
...
if args.rehearse_observed_hosted_gaps:
    ...
    assert len(migration_files)==125 and len(initial_files)==51
