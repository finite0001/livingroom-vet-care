...
assert len(versions) == len(set(versions)) == 128, 'Review canonical restore migration inventory'
...
if args.rehearse_staging_baseline:
    assert len(migration_files) == 128
...
if args.rehearse_observed_hosted_gaps:
    ...
    assert len(migration_files)==128 and len(initial_files)==51
