...
## Repeat the real Storage check

CI runs `scripts/verify-local-storage.mjs` after the SQL tests against its disposable local stack. For a separate local test stack, use `RUN_ISOLATED_STORAGE_TEST=1 SUPABASE_TEST_WORKDIR=/path/to/isolated/stack node scripts/verify-local-storage.mjs`. The script obtains local keys in memory through the Supabase CLI and refuses non-loopback URLs or unexpected ports. It creates a synthetic confirmed user without sending email, household, patient and immutable document history. Reset that disposable stack afterward; do not run it against a working local database. It never contacts the hosted backend or configures real provider credentials.
