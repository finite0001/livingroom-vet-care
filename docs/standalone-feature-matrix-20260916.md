...
| Conversation attachments | **Candidate, not deployed:** PRs150/152/153/156 add private staff uploads, reviewed immutable outbound payloads, shared message history, provider-bound incoming capture, authorized private reads and incoming timeline controls. CI35107565034 passed at843ca92: database/Edge/frontend jobs, 348 browser tests, incoming/outgoing/cleanup concurrency, and 33 incoming HTTP/Auth/RPC/Storage checks. Bounded abandoned-file cleanup and candidate discovery are tested; superseded incoming-file cleanup remains open. Hosted preflight confirms both projects lack attachment tables. Exact schema upgrade rehearsal passed 110 SQL assertions with the historical hosted routine preserved. Populated backup/upgrade, deployed Edge/provider and hosted staff acceptance remain open; see `docs/evidence/hosted-attachment-preflight-20260916.json`. |
