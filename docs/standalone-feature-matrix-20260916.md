...
| Estimates and quotes | Versioned native draft foundation candidate: immutable priced revisions, household editor, history and saved-request resolution (PR155) | **Draft PR155 CI passed:** frontend, Edge and database run35097366692 at02f5fa7; hosted rollout pending. Exact publication PR158 and combined PR157 have passing CI; publication has also passed local SQL, concurrency, actual Auth and selected populated-restore checks ([evidence](evidence/native-estimate-publications-20260916.json)); all seven staff publication browser scenarios also pass. **Still missing:** reviewed delivery, client decisions and accepted-price execution for services, vaccines and medications. See the native estimates plan. |
...
| Conversation attachments | **Deployed to staging; acceptance incomplete.** PRs150/152/153/156 add private uploads, reviewed outbound payloads, incoming capture/reads and timeline controls. Staging now has119 migration receipts, the five attachment handlers and aligned inbound/outbound workers. Actual staging backup/upgrade rehearsal passed213 SQL assertions; synthetic lifecycle restoration covers six states and eight files. Hosted staff API acceptance verifies private upload, deployed byte verification, exact download, anonymous denial and idempotent manifest preparation. Sender/reply settings are absent, so frozen email capture remains unverified and correctly fails closed. The new preview frontend is READY; stable-alias promotion and authenticated UI acceptance await CI. Incoming provider acceptance, superseded incoming-file cleanup, retention configuration and primary rollout remain open. Separate [PR160](https://github.com/finite0001/livingroom-vet-care/pull/160) passed full populated communications database/Storage restore, including original MIME metadata and exact security; its CI/integration remain pending. See [upload evidence](evidence/staging-attachment-upload-acceptance-20260916.json), [worker evidence](evidence/staging-attachment-workers-20260916.json) and [frontend evidence](evidence/staging-attachment-frontend-20260916.json). |
...
Conversation attachments are implemented pending the remaining restore/provider acceptance: staff-owned private uploads, explicit recipient/file review, immutable byte hashes bound to the durable send, attachment-only inbound visibility and authorized retrieval. Then add scheduled delivery with reviewable edit/cancel history and final recipient/consent checks.
