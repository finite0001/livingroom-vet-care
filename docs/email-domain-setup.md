...
The following exact values were supplied by the pending Resend domain UI. Recheck them against the domain's current configuration before entering them. Use GoDaddy's normal/default TTL corresponding to Resend's Auto TTL. Enter host labels as shown rather than appending the domain twice.
