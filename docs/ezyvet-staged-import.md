...
`ezyvet-import` fetches read-only source records into administrator-only staging tables. It never creates, updates, deletes or reconciles Living Room Vet clients, patients, encounters, appointments, invoices or inventory. Review decisions are append-only proposals, not authorization to merge charts. No scheduled jobs, live requests or credentials were commissioned during implementation. The integration UI and approval-to-chart workflow remain separate work.
