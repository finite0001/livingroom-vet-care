...
September13,2026: 57 focused SQL assertions, 143 prior intake/review/release SQL assertions, and 59 runner checks passed. The observed races cover duplicate UUID-independent approvals, competing correction branches, consultation and vaccination source revisions against prepare/approve in both lock orders, household reassignment in both orders, and catalog update in both orders. Holders remain in an explicitly open transaction until a waiter is observed; the harness does not depend on a fixed sleep window. It uses the actual catalog save RPC and restores only its owned synthetic fixture between races. Revoked DVM terminal recovery is denied.

Run `python3 supabase/tests/ezyvet_vaccination_review_concurrency.py --project-config <local config.toml>`. The runner verifies the local Supabase container label, creates an ownership-marked schema-only database, applies5300 when absent, and removes only that verified scratch database in finally. Optional `--overlay-migration PATH --extra-sql-test PATH` supports composed release validation without mutating the foundation database. API/provider access and hosted data are not used. Source-practice samples and Dr.Edler's clinical acceptance remain separate.
