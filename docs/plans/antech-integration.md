...
## Implemented native database foundation (4100)

Migration `20260913410000_lab_result_provenance.sql` supplies the local provenance boundary. It creates no Antech account, assumes no provider identifiers or endpoints, enables no transport, sends no message and interprets no result. The registry starts empty. A report being linked is separate from a veterinarian acknowledging it.

Tables are append-only and accessible through actor-checked RPCs:

- `lab_source_accounts`: administrator-reviewed nonsecret provider/account/environment labels and review note. Manual import is the only enabled capability; transport is constrained off. This records the provenance staff actually reviewed, not vendor commissioning. Corrections to account identity require a new source record.
- `lab_report_receipts`: staff-entered generic source patient/order/report references, receipt time and the exact ready private patient-document identity/version/size/MIME. `receipt_hash` fingerprints staged metadata. No staff-entered SHA is requested and staging does not claim verified bytes.
- `lab_report_byte_captures`: service-only digest of the original private document, with staged metadata hash, document version, size/MIME and a combined `capture_hash`. No report promotion is allowed until this capture exists and its hash is explicitly reviewed. The service must download and hash the exact object; SQL cannot independently inspect Storage bytes.
- `lab_order_source_reviews`: explicit identity-map revisions for a native order, scoped to patient/account/source references, expected native order version and prior mapping head.
- `lab_report_versions`: original report followed by reasoned corrected versions. Each promotion binds a staged receipt, exact metadata and byte-capture hashes, current source mapping, native order version and expected prior report. Corrections require a different ready private document within the original account. Receipts are promoted once; report byte hashes are not used as clinical version identifiers.
- `lab_report_acknowledgments`: independent exact-report acknowledgment by an active DVM. This reuses the existing `DVM` role used by vaccine certificate issuance, not a new inferred clinical role. A prior acknowledgment never covers a later correction, and no import changes a native lab order to `resulted` automatically.

Staff contracts:

1. ADMIN `review_lab_source_account(p_id, p_provider_label, p_account_reference, p_environment_label, p_review_note)` returns the immutable source row.
2. `stage_lab_report_receipt(p_id, p_source_account_id, p_document_id, p_document_version, p_source_patient_reference, p_source_order_reference, p_source_report_reference, p_received_at)` returns the staged receipt. `recover_lab_report_receipt(p_receipt_id)` returns `{receipt, capture, report}`, scoped to the original staging actor; missing capture/report are null. Exact UUID retries recover original records before current-document eligibility. Changed inputs or another actor cannot reuse the UUID.
3. `review_lab_order_source(p_id, p_order_id, p_pet_id, p_expected_order_version, p_source_account_id, p_source_patient_reference, p_source_order_reference, p_previous_review_id, p_review_reason, p_attest)` appends a source-review revision. True attestation is required; the order row serializes mapping changes.
4. `link_lab_report_version(p_id, p_receipt_id, p_expected_receipt_hash, p_expected_capture_hash, p_order_id, p_pet_id, p_expected_order_version, p_source_review_id, p_previous_report_id, p_kind, p_review_reason, p_attest)` returns the immutable version. `p_kind` is `original` or `corrected`. It compares the staged patient/account/references against the current reviewed mapping and locks the order/document before appending.
5. DVM `acknowledge_lab_report(p_id, p_report_id, p_pet_id, p_expected_capture_hash, p_expected_document_version, p_attest)` independently acknowledges exactly that ready report. An ADMIN without DVM is denied. No clinical conclusion, normal-range inference or communication authorization is recorded.
6. `read_lab_result_history(p_pet_id, p_order_id default null)` returns `{pet_id, order_id, sources, staged_receipts, source_reviews, reports}`. Staged receipts include their capture or null. Report versions include capture, current document status and their own acknowledgment array. All reads require active staff and validate patient/order scope. Reports remain in history if a document is later voided; voided files cannot be newly linked or acknowledged.

Service byte-verifier contract: `lab_report_capture_context(p_receipt_id, p_actor_id)` returns `{receipt, document: {id, version, bucket, file_path, mime_type, file_size}, capture}` only for the active staging actor and exact ready document. After downloading and verifying that object, `capture_lab_report_bytes(p_receipt_id, p_actor_id, p_expected_receipt_hash, p_document_version, p_content_sha256, p_file_size, p_mime_type)` captures the immutable digest and returns its row. Altered digests or mismatched metadata cannot replace a capture. Service-only grants enforce the trust boundary; the browser never receives a service key or supplies an accepted digest.

This first staged queue is **unmatched to a native order**, while the original document already belongs to a patient. Unknown-patient provider download staging, externally durable version IDs, parser mappings and provider acknowledgment remain contract-gated future work. Native notes, diagnoses, lab due templates/plans, workflow status and the legacy result-document pointer remain unchanged.

Local verification: 28 focused SQL assertions cover staged-but-unverified denial, exact UUID recovery, service-only digest capture, byte metadata mismatch, cross-patient/account denial, stale order/head checks, original/corrected history, independent DVM acknowledgment, unchanged native records, append-only evidence and inactive/anonymous access. `lab_result_provenance_concurrency.py` passes 13 checks across actual contention for competing original promotions, native order edit versus promotion, and document void versus promotion. SQL tests use explicitly synthetic metadata/digests; actual private byte transport is separate service integration evidence. No provider requests, configured account or client sends are claimed.
