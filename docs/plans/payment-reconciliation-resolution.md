...
Status: **matching-object database workflow implemented in migration 3500**. The original design below is followed by the implemented scope and evidence checkpoint. Audited inbox retry database cycles are implemented in migration 4000 below. Service proof retrieval and administrator UI require separate integration. This document does not commission a provider or authorize a live financial action.
...
Accepted scope is known Checkout objects with `session_open`, `session_expired` or `payment_succeeded`, and known refund objects with `pending`, `failed` or `succeeded`. Only unavailable/reconciliation-required observations and Checkout `provider_reconciliation_required` quarantines can be targeted. Any open context-mismatch observation or other quarantined financial evidence blocks this workflow. Previously failed refunds without settled cash remain outside scope, including fresh matching failed proof; late success after that release remains quarantined. Unknown objects, absence claims, external adjustments and disputes remain outside this workflow. Audited processing retry cycles are separately implemented below.
...
## Audited Stripe inbox retry database (4000)

This increment reopens bounded **processing work only**. It does not apply money, clear any observation/quarantined financial evidence, alter signed receipt attribution or make a provider request.

Authenticated active ADMIN calls `preview_stripe_event_retry(p_receipt_id)` and receives `{receipt, work, eligible, target, expected_work_hash, cycles, history}`. The work projection and history omit lease tokens. `target` is null when ineligible; an eligible target contains invoice/family/request/object identity and the recorded last failure. Worker history includes cycle number, lifetime attempt count and per-cycle count. Historical entries predate recorded lease identities; those missing identities are not invented. Direct staff history reads likewise exclude the new lease-token column.

Discovery remains `read_stripe_event_queue(p_limit default 100)`, capped at 250. It adds `cycle_no` and `cycle_attempt_count` beside the existing lifetime `attempt_count`, preserving the original receipt and queue-state fields.

`requeue_stripe_event(p_resolution_id, p_receipt_id, p_expected_work_hash, p_reason, p_attest)` requires an exact UUID, current work/financial snapshot hash and true attestation. Reasons are the bounded operational review codes `provider_recovered`, `rate_limit_resolved` or `processor_repaired`. Exact replay returns the original immutable cycle receipt before checking current work eligibility; changed UUID inputs fail. The result contains `id, receipt_id, actor_id, cycle_no, expected_work_hash, reason, previous_attempt_count, previous_cycle_attempt_count, created_at`.

Eligibility is deliberately narrow: an originally queued supported receipt, matching commissioned account/mode, a previously accepted exact provider object and the same 3500 mismatch exclusions. The matching-object validator need not have any open reconciliation blockers, so a previously completed financial reconciliation does not prevent processing retry. Work must be quarantined for retry exhaustion after five attempts in its current cycle, with a recorded recoverable provider/processing failure (`provider_unavailable`, `rate_limited`, `transport_unknown`, `processing_error`). Active leases, missing attribution, unsupported events, unresolved financial/context mismatch and terminal-failure refund exclusions remain blocked. Crash-only exhaustion without a recorded recoverable failure remains outside this increment.

Requeue locks work and invoice, rechecks the snapshot/eligibility, appends `stripe_event_retry_cycles` and advances `cycle_no`. Lifetime `attempt_count` never resets. Only the explicit cycle's `cycle_attempt_count` starts at zero, and claim/retry retain a maximum of five attempts and the existing bounded backoff within that cycle. New history records cycle and lease attribution; every earlier row remains immutable. Current lease checks in completion/retry continue to reject old workers. Claim returns the additional cycle fields without changing the provider worker's evidence contract.

Verification: 30 dedicated SQL assertions pass, including two full cycles/ten lifetime attempts, prior successful financial reconciliation, exact UUID recovery, stale hash, old lease denial, no money applied by retry, safe history, ADMIN/active-staff ACLs, unknown object, unsupported event and crash-only exclusion. Existing inbox 44, reconciliation 45, observation 58 and ledger 64 assertions pass. The legacy exhaustion fixture now initializes both lifetime and cycle counters. `stripe_event_retry_concurrency.py` passes 31 checks across competing administrator reviews, an old worker waiting behind requeue, and skip-locked claiming while an administrator holds the work row. Random fixtures are cleaned after the tests. Run it in CI with `--project-config supabase/config.toml`. No provider requests or hosted changes occurred; administrator UI and operational review remain separate work.
