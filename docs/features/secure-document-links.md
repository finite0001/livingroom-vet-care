...
Next increment must add protected staff/client UI and a secure-link outbox subtype with redacted stored wording, exact transient materialization and a mandatory final-attempt proof check that rejects older workers. It must prevent literal capability URLs from entering generic message/outbox storage and recheck eligibility on every delivery attempt. No provider success, clinical acceptance or live public availability is established by these local tests.
