# Secure document links: backend increments 1–3

Implemented locally: immutable capability grants for issued invoices and already confirmed **SMS** medical-record releases, actual frozen report/original capture, explicit staff attestation, revocation, and authenticated public retrieval. The backend now supports explicitly reviewed SMS queueing; it does not establish real provider delivery, supply staff/client UI, or activate a deployment. Existing clinician/schema acceptance remains required for record releases. EMAIL releases cannot be coerced to SMS.
...
Guards reject recognizable canonical `v1.` capabilities in outbound queue/prepared-message/history, inbound provider metadata/inbox history, grant templates, revocation reasons, and email preparations. Incoming SMS signatures are verified against the unchanged original body before redaction; only the redacted body and original body hash are persisted. The existing original event-payload hash remains unchanged for duplicate detection. Provider fetches must match that original body hash, so different private tokens that redact identically cannot be substituted. Older inbound workers attempting to persist canonical tokens fail closed at the database guard.

The inbound processor also redacts canonical tokens from email text, HTML, subject, threading fields and attachment metadata before durable completion. It does not download incoming email or MMS attachment bytes. This is canonical text redaction, not content decoding or malware inspection: arbitrary encoded/obfuscated content, PDFs/images, previously stored records and original patient document bytes are not scrubbed. Original medical attachments may themselves contain links and remain an explicit content-review concern. Local verification does not establish real SMS delivery or authorize provider activation.

Backend verification: the Node suite has 185 passing tests, including 11 focused production-dispatcher tests and three signed-inbound/provider-fetch redaction tests using synthetic transports. Frozen Deno checking passes for dispatch, inbound processing and Twilio webhook endpoints. `supabase/tests/document_link_sms.test.sql` is a rollback-only suite for both grant families, exact review/idempotence, old-worker denial, final source/consent/schema/actor checks and redacted history. All 58 assertions executed successfully against the existing local foundation database after applying migration 2900 and its additive inbound guards without a reset; the test transaction rolled back.

## Staff history selector

`read_document_link_history(p_family,p_source_id)` returns the original active actor's latest 50 grants, newest first by creation time and ID. It returns a JSON array of `{id,created_at,expires_at,state,recipient,receipt_state}` only. `receipt_state` is the current outbox state or null when never queued; grant state and delivery state are separate. The list contains no report, originals, source bundle, capability context, key version or token, and callers cannot choose another actor.

Historical entries remain browseable after source void/withdrawal, phone changes, expiry and revocation. Selecting an entry uses the existing actor-scoped recovery API; new queue/retrieval still requires its full current eligibility checks. Unknown or unowned source pairs produce an empty list. This enables selecting and revoking earlier grants without asking staff to enter internal request UUIDs. The bounded list is the latest 50 entries; it is not an unlimited archive or an administrative cross-staff search.

Migration 3200 was applied only to the existing local foundation database. All 14 rollback-only history assertions pass, covering actor/role isolation, exact metadata keys, family/source separation, the 50-entry cap/order, expired revoked entries and joined failed delivery status.
