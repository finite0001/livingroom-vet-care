...
The API-attachment work is tracked in [draft PR126](https://github.com/finite0001/livingroom-vet-care/pull/126) and [phase3d](../plans/20260913-ezyvet-clinical-import/phase-03d-api-attachments.md). Parent-scoped intake, immutable private capture and an authenticated default-off capture worker are implemented locally. The current90-migration runtime rehearsal passes137 checks, including physical upload/readback, abandoned-file deletion and lost-response recovery. Migration6900 now adds abandoned-only cleanup leases, a narrowly scoped Storage deletion policy and immutable absence receipts. Its41 SQL assertions and the expanded31-check contention suite pass locally; the authenticated default-off cleanup worker now has22 focused tests and actual Storage API acceptance; operator controls remain unfinished. The contention checks also pass against a fully migrated schema clone; broader late-upload ordering and current populated upgrade/restore evidence are still needed. Staff review/release UI, remaining attachment race coverage, current upgrade/restore and provider/clinical acceptance remain open. No hosted attachment migration or worker activation is claimed. Mail/Auth, Stripe sandbox acceptance, vendor integration, clinical and public-cutover gates remain part of the full commercial-readiness goal.
