...
Local migration workspace checkpoint (2026-09-14): the 107-migration candidate now supports operator scope creation, existing-run binding and uncertain-save recovery alongside source/capture evidence. [Interface and actual runtime evidence](evidence/canonical-migration-scope-binding-ui-20260914.json) covers explicit resource decisions, two-patient selection, owned run discovery and shared draft protection. This candidate is not deployed. Explicit resource resume, remaining clinical outcomes/global reconciliation, operational resolutions, frozen reports, supervised staff acceptance and external commissioning remain unfinished. The latest hosted backend remains the 99-migration checkpoint below.
