...
PR147–149 have passing CI for physical returns and shared quantity replay. [Draft PR151](https://github.com/finite0001/livingroom-vet-care/pull/151) adds attributed return corrections, discrepancy decisions, lot holds and current print4/schema13 records. CI35086396244 passed lint, TypeScript,858 unit tests, production build, Edge checks,100 SQL suites/4078 assertions,83 reconciliation contention checks,461 actual native Auth/browser checks and84 populated restore checks. The385-case browser run included one retry success; its saved-history selector is corrected and both affected scenarios passed three fresh repetitions (6/6), with follow-up CI pending. [Draft evidence](evidence/native-return-reconciliation-draft-20260916.json) and [Dr. Edler review checklist](../plans/20260916-native-prescribing/reconciliation-review.md) distinguish these checks from hosted and clinical acceptance. This candidate has not been deployed.
...
Identity approval evidence is deployed to both backends and published in Lovable at merge `5f5d1b1`. CI passed all three jobs; fresh restores of both hosted backups preserved235 tables each and passed30 SQL assertions. Hosted function body/grants and the public application artifact are verified in the [rollout evidence](identity-evidence-rollout-20260915.md). Supervised acceptance remains pending. Weight evidence is also deployed and published at merge `e40dbee`, with CI, hosted function/grant checks, both backup restores and the public artifact verified. Global outcomes remain local and incomplete; resolutions, frozen reports and supervised acceptance remain required.
...
Local operational-decision candidate (2026-09-16): the 114-migration stack now supports reasoned scope and exact-observation exclusions, append-only reopening history, lost-response recovery and patient-scoped weight eligibility. The interface distinguishes current display names from immutable evidence and leaves excluded required coverage incomplete. [Acceptance evidence](evidence/canonical-migration-resolutions-20260916.json) records 616 unit tests, 51 browser tests and 756 disposable checks; the separate supplemental run passed 465 overlapping checks, including 47 observed contention checks. [Populated restore](evidence/migration-resolutions-restore-20260916.json) preserved decision chains and private files; its initial cleanup failed and was recovered with independently verified resource removal. This is local evidence, not a hosted deployment. Further integration/report work is retired by owner direction; native clinical/provider acceptance remains outstanding.
...
The isolated restore rehearsal passed again on September 24, 2026, capturing and restoring all 148 migrations through `20260922230000`. Actual fresh login, private file checksum/access controls, signed clinical history, invoice/credit, stock checks, release artifacts, communications, attachment lifecycle evidence and scheduler containment checks passed; generated resources were verified removed. [The dated restore evidence](launch-evidence/2026-09-24-current-stack-restore-rehearsal.md) records checksums, timings and limits. Local restore success does not establish hosted PITR coverage, production RTO/RPO, provider delivery acceptance or incident cutover readiness.

The earlier September 13 observed-gap rehearsal passed 51→65 migrations with synthetic records present before backfill. All captured clinical/billing/Auth/Storage rows survived unchanged; public function definitions/security/configuration/effective grants and trigger bindings matched a separate canonical installation. Full database/private-Storage restoration and generated-resource cleanup also passed. [Sanitized evidence](hosted-upgrade-preflight.md) records scope and checksums. That paragraph is retained as historical context; the current hosted migration state is the 148-version Sep 24 parity described above.
...
Combined canonical receipt integration: commit `82d4a54` preserves identity migration `20260916010000` and weight migration `20260916033310` together, with113 total migrations. Both panels and actual Auth/runtime checks survive the merge. Local lint/typecheck/build,604 units and37 migration browser tests pass. The frozen complete disposable sequence passes454 checks (metadata50, originals121, maximum137, migration146), including30 identity and52 weight pgTAP assertions. Cleanup and all recorded migration/source hashes were verified. [Combined evidence](evidence/canonical-migration-receipts-combined-20260915.json). No combined populated restore or hosted rollout is claimed; earlier112-migration receipts remain separate evidence.

## Standalone transition and completed local decision package — September 16, 2026
