...
The merged frontend at `c5cd483` is now published in Lovable, including contact storage recovery, canonical-original downloads in record previews, housecall routes and migration mapping-selection fixes. [Publication evidence](live-readiness-release-20260915.md) verifies the public artifact and primary backend. [Hosted pilot evidence](hosted-pilot-rehearsal-20260915.md) covers synthetic household/patient/booking/invoice creation only, not a complete clinical visit or payment.

Local candidate (September 15): prescription-item review evidence now distinguishes selection, omission, source version, parent version, and exact run/page observations. The additive migration brings the local candidate to 111 migrations; hosted environments remain at 110. A fresh disposable stack passed 37 item-evidence SQL assertions and 126 migration HTTP/Auth checks, including the new TypeScript item adapter. Duplicate page observations, changed item payloads, preserved correction history, authorization, and explicit uninterpreted dates are covered. All 31 migration-workspace browser tests passed, alongside 596 unit tests, type checking, lint (one existing AuthContext warning), and build. The populated staging-baseline upgrade and private-Storage restore passed. See [local database/API evidence](evidence/migration-prescription-item-local-20260915.json) and [restore evidence](evidence/migration-prescription-item-restore-20260915.json). PR138 CI exposed a preexisting UTC/Denver date mismatch in the attachment test fixture; the fixture now uses the clinic date without changing the production safeguard. Fresh verification passed all 182 attachment-original assertions, 37 item-evidence assertions and 126 API checks. The full CI rerun remains pending. Deployment and supervised staff acceptance remain pending. This item-level view does not complete whole-migration reconciliation, discrepancy resolution, or frozen reports. No hosted deployment or provider operation was performed.
