...
The API-attachment work is tracked in [draft PR126](https://github.com/finite0001/livingroom-vet-care/pull/126) and [phase3d](../plans/20260913-ezyvet-clinical-import/phase-03d-api-attachments.md). Parent-scoped intake, immutable capture and authenticated default-off capture/cleanup workers are implemented locally. The import page includes patient-scoped scan and file history with source-change notices and pagination, plus explicit saved-scan recovery and one-page continuation controls. Recovery runs before and after each page attempt; uncertain outcomes guard patient switching and navigation. Migration7000 adds bounded owned observation and cleanup-history discovery. The current91-migration rehearsal passes153 HTTP/Auth/Storage checks and31 contention checks; SQL regression passes694 assertions and236 compatibility checks. New scan creation, download preparation/capture/cleanup controls, original inspection, clinical review/corrections and release integration remain unfinished. Current populated upgrade/restore, broader contention and issued-site/operator acceptance are still required. No hosted attachment migration or activation is claimed. Mail/Auth, Stripe sandbox acceptance, vendors, clinical approval and public cutover remain part of the full commercial-readiness goal.
