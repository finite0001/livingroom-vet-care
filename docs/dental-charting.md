...
`PatientDentalChart({ petId, species, onDirtyChange? })` is wired into the patient page. The optional dirty callback reports draft/addendum/pending status. The component confirms discard when switching or reloading charts and warns on browser unload. PatientPage aggregates dental, care-chart (QOL/body map), and clinical (SOAP/problem/addendum) dirty callbacks into one router blocker. Cancel preserves all panel edits; confirmed discard permits navigation. Saving one panel does not clear another panel’s protection. The dental component does not register a competing router blocker.
