...
**Historical deployment note:** at this increment, the clinical-core migration needed to be deployed before the matching frontend because the client list called `search_clients`. Later hosted-readiness evidence supersedes the old `.env` warning: the repository no longer tracks a real `.env`, and the selected hosted backend is recorded in the deployment runbook.
