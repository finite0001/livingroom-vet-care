...
- **Hosted database state:** the hosted project is at 148 matching migrations, 0 remote-only migrations and 0 local-only migrations after the post-PR #204 rollout. Use `npm run supabase:migration-drift --silent` before and after any schema operation.
...
- **Readiness status:** the generated summary remains 4/5 gates passing. Supabase/database, Hub/frontend workflows, external services/deployment and verification/release control pass. The only generated blocker group is owner-approved public-contact content: `phone`, `email` and `emergencyPhone` in `src/config/practice.ts`.
