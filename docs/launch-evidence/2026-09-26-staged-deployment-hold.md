...
Release candidate: `f5a6cd0be3e54663f85d89ceae91e93366356c58` on `codex/commercial-readiness-repair-20260925`, packaged in [PR #205](https://github.com/finite0001/livingroom-vet-care/pull/205). The Vercel preview built. Its URL requires Vercel SSO, so a browser smoke was not completed. No hosted SQL, Edge function, provider setting, or production frontend was changed during this rollout attempt.
...
Do not run the 32-file hosted staging push or mark the two conversation migrations applied from object presence alone. Reconcile the live schema and migration bodies against the canonical 151-migration target, including the receipt-only native reconciliation and finance versions; verify grants, policies, triggers, and existing records. Build a reviewed forward repair or a properly migrated replacement staging project, rehearse its populated upgrade and private Storage recovery, and repeat the access and workflow checks in the [repair progress contract](2026-09-25-repair-progress.md). Only then apply to staging, deploy the explicit matching Edge manifest, test controlled provider round trips, and proceed to the primary project and frontend. Keep payment and live-send gates disabled throughout this release candidate.
