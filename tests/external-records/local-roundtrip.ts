...
const root="http://127.0.0.1:56521",origin="https://thelivingroom.vet";
let server:ReturnType<typeof spawn>|null=null,path="";
try{
 const email=`external-byte-${randomUUID()}@example.test`,password=`Synthetic-${randomUUID()}-Aa1!`;
 ...
 const doc=await staff("prepare_patient_document",{p_id:document,p_pet_id:pet,p_encounter_id:null,p_file_name:"synthetic-external.pdf",p_mime_type:"application/pdf",p_file_size:bytes.length,p_category:"medical_record",p_source:"Synthetic fixture",p_document_date:null,p_visibility:"internal"});path=doc.file_path;
 ...
 const snapshot=randomUUID(),site="Synthetic-"+randomUUID();ids.push(snapshot);
 // An explicitly synthetic approved animal-link fixture; no source network or authenticity claim.
 sql(`insert into public.ezyvet_import_snapshots(id,source_origin,source_site_uid,resource,external_id,payload,payload_hash,first_seen_by) values(${quote(snapshot)},'https://api.trial.ezyvet.com',${quote(site)},'animal','77','{"id":77,"contact_id":8}','synthetic-hash',${quote(actor)});insert into public.ezyvet_record_links(id,request_id,request_hash,source_origin,source_site_uid,resource,external_id,snapshot_id,head_version,client_id,pet_id,local_version,action,reason,approved_by) values(${quote(source)},${quote(randomUUID())},'synthetic-link','https://api.trial.ezyvet.com',${quote(site)},'animal','77',${quote(snapshot)},1,${quote(client)},${quote(pet)},1,'link','SYNTHETIC TEST ONLY',${quote(actor)});`);
 server=spawn("deno",["run","--cached-only","--allow-env","--allow-net=127.0.0.1","tests/external-records/local-server.ts"],{env:{...process.env,SUPABASE_URL:local.API_URL,SUPABASE_SERVICE_ROLE_KEY:local.SERVICE_ROLE_KEY,SUPABASE_ANON_KEY:local.ANON_KEY,APP_URL:origin,EXTERNAL_RECORD_VERIFICATION_ENABLED:"true"},stdio:"ignore"});
 let booted=false;for(let i=0;i<80;i++){if(server.exitCode!==null)throw new Error("Local external-record server failed to boot");try{booted=await(await fetch(root+"/health")).text()==="ready";if(booted)break;}catch{/* Boot pending. */}await new Promise(resolve=>setTimeout(resolve,100));}check(booted,"Production runtime boots with external networking denied");
 ...
 const args={action:"prepare",p_id:receipt,p_animal_link_id:source,p_expected_pet_version:1,p_document_id:document,p_document_version:ready.version,p_export_reference:"Synthetic export",p_received_at:new Date(Date.now()-1000).toISOString(),p_previous_record_id:null,p_review_reason:"Reviewed synthetic patient source"};
 check((await post(args,false)).status===401,"Anonymous byte verification denied");
 check((await post(args)).status===404,"Actual non-admin staff cannot stage external provenance");
 ...
 check((await post({...args,p_export_reference:"Changed source"})).status===409,"Exact SQL receipt conflict cannot recover as success");
 ...
 check(sql(`select count(*) from public.external_record_byte_captures where receipt_id=${quote(receipt)};`)==="1","One durable capture exists");
 check(verified.record===null,"Byte verifier does not approve an external record");
 const approval=randomUUID();ids.push(approval);
 await staff("approve_external_record_import",{p_id:approval,p_receipt_id:receipt,p_expected_receipt_hash:verified.receipt.receipt_hash,p_expected_capture_hash:verified.capture.capture_hash,p_attest:true});
 const approved=await(await post({action:"recover",p_receipt_id:receipt})).json();check(approved.record.id===approval&&approved.record.animal_link_id===source&&approved.record.export_reference===args.p_export_reference,"Separate synthetic approval recovers exact mapping and source intent");
 ...
 check(recovered.record.id===approval,"Historical approved record remains recoverable after document void");
...
if(failures.length)throw new AggregateError(failures,"Local external-record byte verification or cleanup failed");
console.log(`Local external-record HTTP/Auth/Storage/SQL verification: ${assertions} checks passed. Synthetic only; no ezyVet requests.`);
