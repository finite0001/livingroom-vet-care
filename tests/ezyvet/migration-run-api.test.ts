...
test("progress rejects inflated distinct counts, false completion and substituted scope", () => {
  ...
  const value = { version: 2, binding_id: binding.id, scope_id: binding.scope_id, migration_run_id: manifest.run.id, child_run_id: child, resource: "attachment", context_hash: binding.context_hash,
    ...
    attempt_history: { origin: "run_created", started_at: "2026-09-14T12:00:00Z", complete_since_run_creation: true, claims: 1, failed_pages: 0, staged_pages: 1 }, observed_at: "2026-09-14T12:00:00Z" };
...
test("attempt pagination rejects repeated sequences, false baseline origin and service fields", async () => {
  const actor = randomUUID(), scope = randomUUID(), child = randomUUID();
  const binding = parseMigrationBinding({ id: randomUUID(), actor_id: actor, scope_id: scope, child_run_id: child, replaces_id: null, reason: "Attempt",
    context_hash: "a".repeat(64), created_at: "2026-09-14T12:00:00Z", child_context: { version: 1, run_id: child, owner: actor, source_origin: "https://api.trial.ezyvet.com",
      source_site_uid: "site", resource: "attachment", parent_evidence: "exact_parent_version", context: {} } }, actor, scope);
  const event = { id: randomUUID(), sequence: 3, kind: "page_failed", attempt_hash: "b".repeat(64), page: 1, run_status: "running", next_page: 1,
    retry_after: null, error_code: "UPSTREAM_TIMEOUT", staged_item_count: null, history_origin: null, recorded_at: "2026-09-14T12:00:00Z" };
  let events: unknown[] = [event, { ...event, id: randomUUID(), sequence: 2, kind: "claimed", error_code: null }];
  const api = createMigrationRunApi({ async rpc() { return { data: { version: 1, binding_id: binding.id, scope_id: scope, child_run_id: child, events, has_more: true }, error: null }; } }, actor);
  assert.equal((await api.attempts(binding, null, 2)).next_sequence, 2);
  events = [event, { ...event, id: randomUUID() }];
  await assert.rejects(() => api.attempts(binding, null, 2), /transition differs/);
  events = [{ ...event, kind: "baseline" }, { ...event, id: randomUUID(), sequence: 2 }];
  await assert.rejects(() => api.attempts(binding, null, 2), /transition differs/);
  events = [{ ...event, lease_id: randomUUID() }, { ...event, id: randomUUID(), sequence: 2 }];
  await assert.rejects(() => api.attempts(binding, null, 2));
});
