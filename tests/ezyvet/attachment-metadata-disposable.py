...
parser.add_argument('--fixture', choices=['metadata','originals','maximum','migration','sql-only','all'], default='metadata')
...
if not 1025 <= args.api_port <= 65532 or (args.api_port != 62421 and args.fixture not in ('migration','sql-only')):
    parser.error('Alternate port must be 1025–65532 and use migration or sql-only fixture')
if not args.run_synthetic_local:
    parser.error('Explicit --run-synthetic-local required')
if args.fixture == 'sql-only' and not args.extra_sql_test:
    parser.error('sql-only requires at least one --extra-sql-test')
...
selected = ['metadata','originals','maximum','migration'] if args.fixture == 'all' else ([] if args.fixture == 'sql-only' else [args.fixture])
...
harness_hashes = {}
migration_hashes = {}
source_paths = [root / 'src/hub/features/imports/migration-identity-api.ts', root / 'src/hub/features/imports/migration-weight-api.ts', *(p.resolve() for p in args.extra_sql_test), root / 'src/hub/features/imports/migration-prescription-item-api.ts', root / 'src/hub/features/imports/migration-prescription-api.ts', root / 'src/hub/features/imports/migration-vaccination-api.ts', root / 'src/hub/features/imports/migration-history-api.ts', root / 'src/hub/features/imports/migration-resume-api.ts', root / 'src/hub/features/imports/migration-selection-api.ts', root / 'src/hub/features/imports/migration-capture-api.ts', root / 'src/hub/features/imports/migration-items-api.ts', root / 'src/hub/features/imports/migration-run-api.ts', root / 'src/hub/features/imports/attachment-review-history.ts', *sorted((root / 'supabase/functions/_shared').glob('*.ts')), Path(__file__).resolve(), *harness_paths, *sorted((root / 'supabase/functions/ezyvet-import').glob('*.ts')), *sorted((root / 'supabase/functions/capture-ezyvet-attachment').glob('*.ts')), *sorted((root / 'supabase/functions/retrieve-reviewed-ezyvet-original').glob('*.ts')), root / 'src/hub/features/imports/attachment-capture-state.ts', root / 'src/hub/features/imports/attachment-decision-state.ts', root / 'src/hub/features/imports/attachment-review-history.ts']
