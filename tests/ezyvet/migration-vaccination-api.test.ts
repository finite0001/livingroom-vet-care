...
function fixture() {
  ...
  const binding = { id: randomUUID(), scope_id: scope, child_run_id: child, actor_id: actor, replaces_id: null, reason: "Synthetic", created_at: "2026-09-14T12:00:00Z", context_hash: "a".repeat(64),
    child_context: { version: 1 as const, run_id: child, owner: actor, source_origin: "https://api.trial.ezyvet.com" as const, source_site_uid: "site", resource: "vaccination" as const, parent_evidence: "exact_parent_version" as const, context: {} } };
  const item = { page: 1, ordinal: 0, snapshot_id: randomUUID(), evidence_hash: "b".repeat(64) };
  const receipt = { id: randomUUID(), version: 2, version_hash: "c".repeat(64), approved_at: "2026-09-14T12:00:00Z", relationship: "different_source_version", source_current: false, superseded: true, replaces_id: randomUUID(), outside_status: "unknown", administration_date_status: "unknown", next_date_status: "uninterpreted", product_linked: false };
  const page = { version: 1, binding_id: binding.id, scope_id: scope, child_run_id: child, actor_id: actor, page: item.page, snapshot_id: item.snapshot_id, evidence_hash: item.evidence_hash,
    visibility: "approved_patient_vaccination", approvals: [receipt], has_more: false, next_before_version: null, local_administration_verified: false, due_plan_adoption_verified: false, complete_coverage_verified: false, observed_at: "2026-09-14T12:00:00Z" };
  return { actor, binding, item, receipt, page };
}
test("vaccination receipts retain source drift, corrections and unknown interpretation as separate facts", async () => {
  const f = fixture(); let params;
  const api = createMigrationVaccinationApi({ async rpc(name, args) { assert.equal(name, "list_ezyvet_migration_vaccination_evidence"); params = args; return { data: f.page, error: null }; } }, f.actor);
  assert.deepEqual(await api.list(f.binding, f.item), f.page);
  assert.equal(params.p_evidence_hash, f.item.evidence_hash);
});
test("vaccination receipts reject foreign identity, clinical payload leakage and false administration claims", async () => {
  const f = fixture();
  for (const page of [{ ...f.page, actor_id: randomUUID() }, { ...f.page, snapshot_id: randomUUID() }, { ...f.page, binding_id: randomUUID() }, { ...f.page, complete_coverage_verified: true },
    { ...f.page, approvals: [{ ...f.receipt, original: "Private prose" }] }, { ...f.page, local_administration_verified: true }, { ...f.page, due_plan_adoption_verified: true }, { ...f.page, approvals: [{ ...f.receipt, outside_status: "given_here" }] }, { ...f.page, approvals: [{ ...f.receipt, replaces_id: null }] }, { ...f.page, next_before_version: 2 }]) {
    const api = createMigrationVaccinationApi({ async rpc() { return { data: page, error: null }; } }, f.actor);
    await assert.rejects(() => api.list(f.binding, f.item));
  }
});
test("vaccination version pagination rejects repeats, duplicates and false continuation", async () => {
...
test("vaccination receipt reads reject invalid inputs before transport and propagate database denial", async () => {
  const f = fixture(); let calls = 0; const error = { code: "42501" };
  const api = createMigrationVaccinationApi({ async rpc() { calls++; return { data: null, error }; } }, f.actor);
