...
const directory = await rpc('list_native_prescribers', { p_after_id: null, p_limit: 1 });
...
assert.deepEqual(await rpc('recover_native_prescription_operation', { p_id: saveId }, staff.headers), saved); checks++;
...
const preview = await rpc('preview_native_prescription_sign', { p_draft_id: draftId, p_expected_version: 1 });
...
assert.deepEqual(await rpc('recover_native_prescription_operation', { p_id: signId }), signed); checks++;
assert.deepEqual(await rpc('read_native_prescription_authorization', { p_id: signId, p_pet_id: patient.id }), signed.result); checks++;
...
await denied('save_native_prescription_draft', { p_id: randomUUID(), p_request: { ...saveRequest, expected_version: 1 } }, '40001', staff.headers);
const listing = await rpc('list_native_prescription_drafts', { p_pet_id: patient.id, p_before_at: null, p_before_id: null, p_limit: 20 });
