...
test('housecall booking shows alerts, retains conflicts and saves Denver instant', async ({ page }, testInfo) => {
 const user = { id: staff, aud: 'authenticated', role: 'authenticated', email: 'synthetic@example.test', app_metadata: {}, user_metadata: {}, created_at: '2026-01-01T00:00:00Z' };
 ...
 const session = { access_token: token, refresh_token:'synthetic',token_type:'bearer',expires_in:3600,expires_at:expires,user };
 await page.addInitScript(value => localStorage.setItem('sb-127-auth-token',JSON.stringify(value)),session);
 let conflict = true; let saves = 0;
 ...
 await page.route('http://127.0.0.1:54321/**', async route => {
  ...
  if(path === '/rest/v1/rpc/save_appointment') {
   saves++; if(conflict) { conflict=false; return route.fulfill({status:409,json:{code:'23P01',message:'Clinician or room is already booked, including travel buffers'}}); }
   const body = route.request().postDataJSON();
   expect(body.p_scheduled_at).toBe('2026-10-28T15:00:00.000Z'); expect(body.p_actor_id).toBe(staff); expect(body.p_address_snapshot).toBe('100 Synthetic Street, Boulder, CO'); expect(body.p_reminder_offsets).toEqual([48,24]);
 ...
 await page.getByLabel('Date and time (America/Denver)',{exact:true}).fill('2026-10-28T09:00');
 await page.getByLabel('Location',{exact:true}).selectOption('housecall');
 await expect(page.getByLabel('Visit address')).toHaveValue('100 Synthetic Street, Boulder, CO');
 await expect(page.getByRole('link',{name:'Open directions in Google Maps'})).toHaveAttribute('href',/destination=100/);
 ...
 await page.screenshot({path:testInfo.outputPath('housecall-booking.png'),fullPage:true});
 await page.getByRole('button',{name:'Save appointment',exact:true}).click();
