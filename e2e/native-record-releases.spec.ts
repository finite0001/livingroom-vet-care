...
async function fixture(page: Page) {
  ...
  const sources = () => ({
    ...
    policy_v10_accepted: state.accepted,
    ...emptySelection(),
  ...
  await page.route("http://127.0.0.1:54321/**", async (route) => {
    ...
    if (name === "list_record_release_sources_v10")
      return route.fulfill({ json: sources() });
    if (name === "select_all_record_release_sources_v10") {
    ...
    if (name === "preview_record_release_v10") {
...
test("policy9 does not authorize schema10 confirmation and selected cancelled order is historical", async ({
  page,
}) => {
  const state = await fixture(page);
