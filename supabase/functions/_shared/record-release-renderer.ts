import { renderReleaseApiOriginals, type ReleaseApiOriginal } from "./record-release-api-originals.ts";
export type { ReleaseApiOriginal, ReleaseApiOriginalAcknowledgment } from "./record-release-api-originals.ts";
...
export interface ReleaseSnapshot extends ChartSources, HistorySources {
  ...
  api_originals?: ReleaseApiOriginal[];
...
export function renderRecordRelease(artifact: ReleaseArtifact): string {
  ...
  if (
    s.encounters.length +
      ...
      (s.schema_version === 9 ? s.api_originals?.length || 0 : 0) +
      (s.weights?.length || 0) +
