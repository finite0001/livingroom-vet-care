-- Patient-scoped outside prescription headers. No approval, dispensing or billing.
alter table public.ezyvet_import_runs drop constraint ezyvet_import_runs_resource_check;
alter table public.ezyvet_import_runs add constraint ezyvet_import_runs_resource_check check(resource in ('contact','contactdetail','address','animal','species','breed','sex','animalcolour','appointment','consult','history','vaccination','diagnostic','healthstatus','prescription','prescriptionitem'));
-- Patient-scoped source observations only; no native prescription promotion.
create table public.ezyvet_prescription_runs (
 ...
 source_origin text not null,source_site_uid text not null,resource text not null check(resource in ('prescription')),
 animal_external_id text not null check(animal_external_id ~ '^[0-9]+$'),created_at timestamptz not null default now()
);
create table public.ezyvet_prescription_pages (
 run_id uuid not null references public.ezyvet_prescription_runs(run_id),page integer not null,
...
create table public.ezyvet_prescription_page_observations (
 ...
 foreign key(run_id,page) references public.ezyvet_prescription_pages(run_id,page),
 foreign key(run_id,page,snapshot_id) references public.ezyvet_import_page_items(run_id,page,snapshot_id)
);
do $$declare t text;begin foreach t in array array['ezyvet_prescription_runs','ezyvet_prescription_pages','ezyvet_prescription_page_observations'] loop
 execute format('alter table public.%I enable row level security',t);
 execute format('revoke all on table public.%I from public,anon,authenticated,service_role',t);
 execute format('create trigger immutable_prescription_import before update or delete on public.%I for each row execute function public.guard_inquiry_history()',t);
end loop;end $$;
...
create function public.claim_ezyvet_prescription_import(p_id uuid,p_actor uuid,p_site_uid text,p_resource text,p_source_origin text,p_animal_link_id uuid) returns jsonb language plpgsql security definer set search_path=public as $$
...
alter function public.stage_ezyvet_import_page(uuid,uuid,uuid,integer,boolean,jsonb) rename to stage_ezyvet_import_page_pre_prescription;
revoke all on function public.stage_ezyvet_import_page_pre_prescription(uuid,uuid,uuid,integer,boolean,jsonb) from public,anon,authenticated,service_role;
...
create function public.ezyvet_prescription_run_projection(p_id uuid) returns jsonb language sql stable security definer set search_path=public as $$
 select jsonb_build_object('id',r.id,'requested_by',r.requested_by,'resource',r.resource,'source_origin',r.source_origin,'source_site_uid',r.source_site_uid,'status',r.status,'next_page',r.next_page,'retry_after',r.retry_after,'last_error_code',r.last_error_code,'created_at',r.created_at,'updated_at',r.updated_at,'lease_active',coalesce(r.lease_until>now(),false),'scope',case when c.run_id is null then 'legacy_unscoped' else 'patient_scoped' end,'animal_link_id',c.animal_link_id,'animal_external_id',c.animal_external_id,'pet_id',c.pet_id,'client_id',c.client_id)
 from public.ezyvet_import_runs r left join public.ezyvet_prescription_runs c on c.run_id=r.id where r.id=p_id;
$$;
create function public.recover_ezyvet_prescription_run(p_id uuid,p_animal_link_id uuid,p_resource text) returns jsonb language plpgsql stable security definer set search_path=public as $$
declare r public.ezyvet_import_runs;c public.ezyvet_prescription_runs;m public.ezyvet_record_links;
begin
 ...
 if p_id is null or p_animal_link_id is null or p_resource is null or p_resource not in ('prescription') then raise exception 'Prescription recovery identity required' using errcode='23514';end if;
 select * into r from public.ezyvet_import_runs where id=p_id;
 if not found then return null;end if;
 select * into m from public.ezyvet_record_links where id=p_animal_link_id and resource='animal';
 select * into c from public.ezyvet_prescription_runs where run_id=p_id;
 if r.requested_by is distinct from auth.uid() or r.resource is distinct from p_resource or row(r.source_origin,r.source_site_uid) is distinct from row(m.source_origin,m.source_site_uid) or (c.run_id is not null and c.animal_link_id is distinct from p_animal_link_id) then raise exception 'Prescription recovery identity mismatch' using errcode='42501';end if;
 return public.ezyvet_prescription_run_projection(p_id);
end $$;
create function public.list_ezyvet_prescription_runs(p_animal_link_id uuid,p_resource text,p_before_at timestamptz default null,p_before_id uuid default null,p_limit integer default 20) returns jsonb language plpgsql stable security definer set search_path=public as $$
declare m public.ezyvet_record_links;items jsonb;more boolean;lastrow jsonb;
begin
 if public.ezyvet_is_active_admin(auth.uid()) is not true then raise exception 'Active administrator required' using errcode='42501';end if;
 if p_resource is null or p_resource not in ('prescription') or p_limit is null or p_limit not between 1 and 50 or (p_before_at is null)<>(p_before_id is null) then raise exception 'Invalid prescription run query' using errcode='23514';end if;
 select * into strict m from public.ezyvet_record_links where id=p_animal_link_id and resource='animal';
 select coalesce(jsonb_agg(public.ezyvet_prescription_run_projection(id) order by created_at desc,id desc),'[]') into items from (
  select r.id,r.created_at from public.ezyvet_import_runs r left join public.ezyvet_prescription_runs c on c.run_id=r.id
  where r.requested_by=auth.uid() and r.resource=p_resource and r.source_origin=m.source_origin and r.source_site_uid=m.source_site_uid and (c.run_id is null or c.animal_link_id=m.id)
...
create function public.list_ezyvet_prescription_candidates(p_animal_link_id uuid,p_resource text,p_before_at timestamptz default null,p_before_id uuid default null,p_limit integer default 20) returns jsonb language plpgsql stable security definer set search_path=public as $$
declare m public.ezyvet_record_links;items jsonb;more boolean;lastrow jsonb;
begin
 if public.ezyvet_is_active_admin(auth.uid()) is not true then raise exception 'Active administrator required' using errcode='42501';end if;
 if p_resource is null or p_resource not in ('prescription') or p_limit is null or p_limit not between 1 and 50 or (p_before_at is null)<>(p_before_id is null) then raise exception 'Invalid prescription candidate query' using errcode='23514';end if;
 select * into strict m from public.ezyvet_record_links where id=p_animal_link_id and resource='animal';
 select coalesce(jsonb_agg(item order by created_at desc,id desc),'[]') into items from (
  select s.id,s.created_at,jsonb_build_object('id',s.id,'payload',s.payload,'payload_hash',s.payload_hash,'external_id',s.external_id,'resource',s.resource,'source_origin',s.source_origin,'source_site_uid',s.source_site_uid,'created_at',s.created_at,'head_version',h.version,'current_snapshot_id',h.snapshot_id,'is_current',h.snapshot_id=s.id and h.version=observed.version,'is_current_snapshot',h.snapshot_id=s.id,'observed_head_version',observed.version,'current_head_scoped',h.snapshot_id=s.id and h.version=observed.version,'animal_link_id',m.id,'pet_id',m.pet_id,'client_id',m.client_id) item
  ...
  join lateral (select max(o.head_version) version from public.ezyvet_prescription_page_observations o join public.ezyvet_prescription_runs c on c.run_id=o.run_id where o.snapshot_id=s.id and c.animal_link_id=m.id) observed on observed.version is not null
  where s.resource=p_resource and s.source_origin=m.source_origin and s.source_site_uid=m.source_site_uid and s.payload->>'animal_id'=m.external_id
  and exists(select 1 from public.pets p where p.id=m.pet_id and p.client_id=m.client_id)
  and exists(select 1 from public.ezyvet_import_page_items i join public.ezyvet_prescription_pages cp on cp.run_id=i.run_id and cp.page=i.page join public.ezyvet_prescription_runs c on c.run_id=cp.run_id where i.snapshot_id=s.id and c.animal_link_id=m.id and row(c.resource,c.source_origin,c.source_site_uid,c.animal_external_id,c.pet_id,c.client_id)=row(s.resource,s.source_origin,s.source_site_uid,m.external_id,m.pet_id,m.client_id))
 ...
 more:=jsonb_array_length(items)>p_limit;items:=case when more then items-p_limit else items end;lastrow:=items->(jsonb_array_length(items)-1);
 return jsonb_build_object('animal_link_id',m.id,'resource',p_resource,'candidates',items,'has_more',more,'next_cursor',case when more then jsonb_build_object('before_at',lastrow->'created_at','before_id',lastrow->'id') else null end);
end $$;
do $$declare f record;begin
 for f in select oid::regprocedure signature,proname from pg_proc where pronamespace='public'::regnamespace and proname in ('claim_ezyvet_import','claim_ezyvet_prescription_import','stage_ezyvet_import_page','ezyvet_prescription_run_projection','recover_ezyvet_prescription_run','list_ezyvet_prescription_runs','list_ezyvet_prescription_candidates') loop
  execute format('revoke all on function %s from public,anon,authenticated,service_role',f.signature);
  if f.proname in ('claim_ezyvet_import','claim_ezyvet_prescription_import','stage_ezyvet_import_page') then execute format('grant execute on function %s to service_role',f.signature);
  elsif f.proname<>'ezyvet_prescription_run_projection' then execute format('grant execute on function %s to authenticated',f.signature);end if;
 end loop;
end $$;

create index ezyvet_prescription_runs_mapping on public.ezyvet_prescription_runs(animal_link_id,created_at desc,run_id desc);
create index ezyvet_prescription_observations_snapshot on public.ezyvet_prescription_page_observations(snapshot_id,head_version desc);
