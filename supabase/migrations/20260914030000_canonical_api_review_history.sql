...
declare actor uuid:=auth.uid();r public.ezyvet_attachment_capture_requests;m uuid;external_id text;items jsonb;lastrow jsonb;more boolean;latest uuid;
begin
 ...
 select * into r from public.ezyvet_attachment_capture_requests where id=p_request_id;
 if r.id is null or row(r.requested_by,r.pet_id,r.status) is distinct from row(actor,p_pet_id,'ready'::text) then raise exception 'Owned captured request required' using errcode='42501';end if;
 ...
 select coalesce(jsonb_agg(to_jsonb(v)-'source_context' order by v.created_at desc,v.id desc),'[]') into items
