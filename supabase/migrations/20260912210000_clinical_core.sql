-- Clinical history is retained; signed encounters and addenda cannot be rewritten.
...
-- Legacy DOB values remain useful for age display, but their original certainty is unverified.
update public.pets set birth_date_precision='estimated' where dob is not null;
alter table public.pets drop constraint pets_client_id_fkey;
