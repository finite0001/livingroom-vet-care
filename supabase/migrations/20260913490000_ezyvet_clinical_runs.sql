-- Patient-scoped source observations only; no native clinical promotion.
create table public.ezyvet_clinical_runs (
 ...
 source_origin text not null,source_site_uid text not null,resource text not null check(resource in ('consult','history')),
 animal_external_id text not null check(animal_external_id ~ '^[0-9]+$'),created_at timestamptz not null default now()
);
create table public.ezyvet_clinical_pages (
 run_id uuid not null references public.ezyvet_clinical_runs(run_id),page integer not null,
...
create table public.ezyvet_clinical_page_observations (
 ...
 foreign key(run_id,page) references public.ezyvet_clinical_pages(run_id,page),
 foreign key(run_id,page,snapshot_id) references public.ezyvet_import_page_items(run_id,page,snapshot_id)
);
do $$declare t text;begin foreach t in array array['ezyvet_clinical_runs','ezyvet_clinical_pages','ezyvet_clinical_page_observations'] loop
 execute format('alter table public.%I enable row level security',t);
 execute format('revoke all on table public.%I from public,anon,authenticated,service_role',t);
 execute format('create trigger immutable_clinical_import before update or delete on public.%I for each row execute function public.guard_inquiry_history()',t);
end loop;end $$;
...
begin
 if p_resource in ('healthstatus','diagnostic','consult','history') then raise exception 'Resource requires a supported patient-scoped contract' using errcode='23514';end if;
 return public.claim_ezyvet_import_core(p_id,p_actor,p_site_uid,p_resource,p_source_origin);
end $$;
create function public.claim_ezyvet_clinical_import(p_id uuid,p_actor uuid,p_site_uid text,p_resource text,p_source_origin text,p_animal_link_id uuid) returns jsonb language plpgsql security definer set search_path=public as $$
...
alter function public.stage_ezyvet_import_page(uuid,uuid,uuid,integer,boolean,jsonb) rename to stage_ezyvet_import_page_pre_clinical;
revoke all on function public.stage_ezyvet_import_page_pre_clinical(uuid,uuid,uuid,integer,boolean,jsonb) from public,anon,authenticated,service_role;
...
create function public.ezyvet_clinical_run_projection(p_id uuid) returns jsonb language sql stable security definer set search_path=public as $$
 ...
 from public.ezyvet_import_runs r left join public.ezyvet_clinical_runs c on c.run_id=r.id where r.id=p_id;
$$;
create function public.search_ezyvet_mapped_patients(p_search text,p_limit integer default 20) returns table(link_id uuid,pet_id uuid,patient_name text,household_name text,source_origin text,source_site_uid text,external_id text,patient_version integer) language sql stable security definer set search_path=public as $$
 select * from public.search_ezyvet_weight_patients(p_search,p_limit);
$$;
create function public.recover_ezyvet_clinical_run(p_id uuid,p_animal_link_id uuid,p_resource text) returns jsonb language plpgsql stable security definer set search_path=public as $$
declare r public.ezyvet_import_runs;c public.ezyvet_clinical_runs;m public.ezyvet_record_links;
begin
 ...
 if p_id is null or p_animal_link_id is null or p_resource is null or p_resource not in ('consult','history') then raise exception 'Clinical recovery identity required' using errcode='23514';end if;
 select * into r from public.ezyvet_import_runs where id=p_id;
 if not found then return null;end if;
 select * into m from public.ezyvet_record_links where id=p_animal_link_id and resource='animal';
 select * into c from public.ezyvet_clinical_runs where run_id=p_id;
 if r.requested_by is distinct from auth.uid() or r.resource is distinct from p_resource or row(r.source_origin,r.source_site_uid) is distinct from row(m.source_origin,m.source_site_uid) or (c.run_id is not null and c.animal_link_id<>p_animal_link_id) then raise exception 'Clinical recovery identity mismatch' using errcode='42501';end if;
 return public.ezyvet_clinical_run_projection(p_id);
end $$;
create function public.list_ezyvet_clinical_runs(p_animal_link_id uuid,p_resource text,p_before_at timestamptz default null,p_before_id uuid default null,p_limit integer default 20) returns jsonb language plpgsql stable security definer set search_path=public as $$
declare m public.ezyvet_record_links;items jsonb;more boolean;lastrow jsonb;
begin
 if public.ezyvet_is_active_admin(auth.uid()) is not true then raise exception 'Active administrator required' using errcode='42501';end if;
 if p_resource is null or p_resource not in ('consult','history') or p_limit is null or p_limit not between 1 and 50 or (p_before_at is null)<>(p_before_id is null) then raise exception 'Invalid clinical run query' using errcode='23514';end if;
 select * into strict m from public.ezyvet_record_links where id=p_animal_link_id and resource='animal';
 select coalesce(jsonb_agg(public.ezyvet_clinical_run_projection(id) order by created_at desc,id desc),'[]') into items from (
  select r.id,r.created_at from public.ezyvet_import_runs r left join public.ezyvet_clinical_runs c on c.run_id=r.id
...
create function public.list_ezyvet_clinical_candidates(p_animal_link_id uuid,p_resource text,p_before_at timestamptz default null,p_before_id uuid default null,p_limit integer default 20) returns jsonb language plpgsql stable security definer set search_path=public as $$
declare m public.ezyvet_record_links;items jsonb;more boolean;lastrow jsonb;
begin
 if public.ezyvet_is_active_admin(auth.uid()) is not true then raise exception 'Active administrator required' using errcode='42501';end if;
 if p_resource is null or p_resource not in ('consult','history') or p_limit is null or p_limit not between 1 and 50 or (p_before_at is null)<>(p_before_id is null) then raise exception 'Invalid clinical candidate query' using errcode='23514';end if;
 select * into strict m from public.ezyvet_record_links where id=p_animal_link_id and resource='animal';
 select coalesce(jsonb_agg(item order by created_at desc,id desc),'[]') into items from (
  ...
  join lateral (select max(o.head_version) version from public.ezyvet_clinical_page_observations o join public.ezyvet_clinical_runs c on c.run_id=o.run_id where o.snapshot_id=s.id and c.animal_link_id=m.id) observed on observed.version is not null
...
do $$declare f record;begin
 for f in select oid::regprocedure signature,proname from pg_proc where pronamespace='public'::regnamespace and proname in ('claim_ezyvet_import','claim_ezyvet_clinical_import','stage_ezyvet_import_page','ezyvet_clinical_run_projection','search_ezyvet_mapped_patients','recover_ezyvet_clinical_run','list_ezyvet_clinical_runs','list_ezyvet_clinical_candidates') loop
  execute format('revoke all on function %s from public,anon,authenticated,service_role',f.signature);
  if f.proname in ('claim_ezyvet_import','claim_ezyvet_clinical_import','stage_ezyvet_import_page') then execute format('grant execute on function %s to service_role',f.signature);
  elsif f.proname<>'ezyvet_clinical_run_projection' then execute format('grant execute on function %s to authenticated',f.signature);end if;
 end loop;
end $$;
