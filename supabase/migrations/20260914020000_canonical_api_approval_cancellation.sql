...
create table public.ezyvet_attachment_approval_cancellations (
 id uuid primary key,actor_id uuid not null references public.profiles(id),request_id uuid not null references public.ezyvet_attachment_original_captures(request_id),
...
declare actor uuid:=auth.uid();v public.ezyvet_attachment_record_versions;c public.ezyvet_attachment_approval_cancellations;r record;
begin
 if public.ezyvet_is_active_admin(actor) is not true then raise exception 'Active administrator required' using errcode='42501';end if;
 ...
 if found then
  ...
  select cap.request_id,cap.capture_hash,q.requested_by as actor_id,q.pet_id into r from public.ezyvet_attachment_original_captures cap join public.ezyvet_attachment_capture_requests q on q.id=cap.request_id where cap.request_id=p_request_id;
