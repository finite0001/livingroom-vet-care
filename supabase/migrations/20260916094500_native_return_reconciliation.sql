...
create table public.native_return_discrepancy_correction_links(id uuid primary key default gen_random_uuid(),case_id uuid not null references public.native_return_discrepancy_events(id),correction_id uuid not null unique references public.native_return_events(id),resolution_id uuid not null references public.native_return_discrepancy_events(id));
...
begin
 ...
 select coalesce(jsonb_agg(lot_id order by lot_id),'[]'::jsonb) into lots from(select distinct case_allocation.value->>'lot_id' lot_id from jsonb_array_elements(rows) as case_entry(value) cross join lateral jsonb_array_elements(case_entry.value->'allocations') as case_allocation(value) where case_entry.value->>'status'='open')q;
...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object('id',r.id,'sequence',r.sequence,'action',r.action,'intake_id',r.intake_id,'correction_target_id',r.document#>'{correction_target,event_id}','allocations',(select jsonb_agg(allocation_row.value-'movement_id' order by allocation_row.value->>'allocation_id') from jsonb_array_elements(r.document->'allocations') as allocation_row(value))) order by r.sequence),'[]'::jsonb) into quantity_events from public.native_return_events r where r.dispense_id=p_dispense_id;
...
declare a uuid:=public.clinical_require_staff();aid uuid;pet uuid;did uuid;v jsonb;r jsonb;d jsonb;i jsonb;policy jsonb;stock jsonb;lots jsonb:='[]';blockers jsonb:='[]';product public.catalog_products;lot public.inventory_lots;x jsonb;b jsonb;ctx jsonb;correction jsonb;practice date;source public.native_return_events;quantity_events jsonb;originals jsonb;lines jsonb:='[]';act text;candidate jsonb;begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object('id',q->'id','sequence',q->'sequence','action',q->'action','intake_id',q->'intake_id','correction_target_id',q#>'{correction_target,event_id}','allocations',(select jsonb_agg(allocation_row.value-'movement_id' order by allocation_row.value->>'allocation_id') from jsonb_array_elements(q->'allocations') as allocation_row(value))) order by (q->>'sequence')::integer),'[]'::jsonb) into quantity_events from jsonb_array_elements(v->'events')q;
...
declare a uuid:=public.clinical_require_staff();aid uuid;pet uuid;did uuid;act text;v jsonb;r jsonb;source public.native_return_events;case_row jsonb;x jsonb;prior text:='';u text;lines jsonb:='[]';ctx jsonb;blockers jsonb:='[]';lot record;begin
 ...
 if act='resolve_corrected' then
  ...
  if exists(select 1 from jsonb_array_elements(lines)q where (q->>'quantity')::numeric is distinct from(select sum((correction_allocation.value->>'quantity')::numeric) from public.native_return_events e cross join lateral jsonb_array_elements(e.document->'allocations') as correction_allocation(value) where p_intent->'correction_ids' ? e.id::text and correction_allocation.value->>'allocation_id'=q->>'allocation_id'))
   or exists(select 1 from public.native_return_events e cross join lateral jsonb_array_elements(e.document->'allocations') as correction_allocation(value) where p_intent->'correction_ids' ? e.id::text and not exists(select 1 from jsonb_array_elements(lines)q where q->>'allocation_id'=correction_allocation.value->>'allocation_id')) then raise exception 'Correction quantities must exactly reconcile case' using errcode='23514';end if;
