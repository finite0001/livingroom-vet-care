...
try:
    ...
    for version in ['20260913520000', '20260913530000', '20260913540000', '20260913900000'] + [f'20260913{n}0000' for n in range(55, 63)]:
    ...
    before = snapshot()
    check(len(before['public.ezyvet_imported_prescriptions']) == 2, 'Pre-upgrade approved correction chain populated')
    ...
    receipt = {'scope': 'local populated pre6300/6400 upgrade only', 'checks': checks, 'table_count': len(before), 'row_count': sum(map(len, before.values())), 'migrations': {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in upgrade_paths}, 'hosted_changes': False, 'provider_requests': 0}
finally:
