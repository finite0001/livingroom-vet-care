...
try:
    ...
    try:
        contended('lock table ezyvet_attachment_page_observations in access exclusive mode;',staff+f"select read_ezyvet_migration_binding_progress('{binding_id}');",lambda code,out,err:code!=0 and 'Active administrator required' in err,during_wait=lambda:sql(f"update profiles set is_active=false where id='{actor}';"))
    finally:
        sql(f"update profiles set is_active=true where id='{actor}';")
    def prepare(rid):
