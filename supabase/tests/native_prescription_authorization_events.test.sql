...
select is(read_native_prescription_status((select id from fx where k='sign'),(select id from fx where k='pet'))->'usage','{"version":2,"native_fill_accounting":"implemented","dispensed_quantity":"0.000","used_fill_slots":0,"remaining_quantity":"90.000","forfeited_quantity":"0.000","unopened_fill_slots":3,"allowance_basis":"native_practice_stock","open_slot":null,"fulfillment_head":{"event_id":null,"version":0},"external_fulfillment":"unknown"}'::jsonb,'Current native allowance is counted while external fulfillment remains unknown');
select throws_ok($$select read_native_prescription_print((select id from fx where k='sign'),gen_random_uuid())$$,'23514','Exact native authorization dispense required','No fabricated dispense print');
