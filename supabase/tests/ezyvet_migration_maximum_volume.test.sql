...
create temp table max_timings(binding_id uuid,first_page integer,elapsed_ms numeric);
-- Each generated statement retains its own ten-second timeout. No sampling or truncation.
select format($query$do $window$ declare started timestamptz:=clock_timestamp();begin
 ...
 insert into max_timings values(%L,%s,extract(epoch from clock_timestamp()-started)*1000);
end $window$; /* LRV_SCALE_WINDOW %s */$query$,
 p.id,b.id,g.first_page,p.id,b.id,g.first_page,b.id,g.first_page,row_number() over(order by s.n,g.first_page))
...
select diag(jsonb_build_object('source_windows',count(*),'total_window_ms',sum(elapsed_ms),'slowest_window_ms',max(elapsed_ms),
